...
BSTNode* llrbtree::moveRedLeft(BSTNode* h) {
    flipColors(h);
...
BSTNode* llrbtree::moveRedRight(BSTNode* h) {
    flipColors(h);
...
BSTNode* llrbtree::deleteNode(BSTNode* h, int data) {
    if(data < h->data){
        ...
        if(data == h->data){
            h->data = find_ios(h->right, h->right)->data;
...
BSTNode* llrbtree::deleteMin(BSTNode* h) {
    if(h->left == nullptr){
        return nullptr;
    }    
    if(!isRed(h->left) && !isRed(h->left->left)){
...
BSTNode* llrbtree::fixUp(BSTNode* h) {
    // Implement your fixUp logic here
    // This method should perform necessary LLRB rotations and color flips
    return h;
}
...
BSTNode* llrbtree::remove(int data, BSTNode* root) {
    ...
    // Ensure that there are no consecutive red nodes on the left
    if(isRed(root->left) && isRed(root->left->left)){
        root = rotateRight(root);
    }
